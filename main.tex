...
The results demonstrate the effectiveness DQL-based approaches for task offloading in hierarchical edge-fog-cloud environments. The MLP-based DQL model, while computationally efficient, exhibits limitations in capturing complex task-node interactions, leading to less stable convergence and suboptimal performance compared to transformer-based methods. In contrast, the NOTE and T-NOTE architectures leverage self-attention mechanisms to effectively model inter-node relationships and task-specific contexts, resulting in superior performance across all evaluation metrics. However, the increased performance comes with an additional computational complexity, especially for transformer-based models, which may limit their applicability in resource-constrained environments.
The DQL suffers also of the fact that it require experimental reward weight tuning to achieve the desired trade-off between TTR, latency, and energy consumption. Reward weights tuning does not always follow a linear relationship with the performance metrics and intuitive behavior, making it challenging to predict the exact impact of weight adjustments on the overall performance. These trade-offs can lead to unexpected outcomes, where small changes in weights result in significant performance variations, complicating the optimization process.
...
Including task-level features in the T-NOTE architecture conduct to surprising resuts in terms of offloading strategies. The edge node is widely is utilized, as shown in Figure~\ref{fig:T-NOTE-offloading-stats}, which is not the case for other approaches. It also 
\begin{figure}[H]
    \centering
